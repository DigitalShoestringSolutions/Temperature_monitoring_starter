...
def do_run(conf, mqttClient, topic):
    ...
    while True:
        ...
        msg_str = {"Time":datetime.datetime.now(tz=tz).isoformat(),"temp":str(AmbientTemp),
          "Threshold":str(Threshold),
          "AlertVal":str(AlertVal)
        }
        
        publish(mqttClient, msg_str, topic)
        time.sleep(1)
...
def run():
    conf = get_config()
    machine_name = conf['machine'].get('name',"Machine Name Not Set")
    mqttClient = connect_mqtt(machine_name)
    mqttClient.loop_start()
    topic = conf['topic']['name']
    do_run(conf, mqttClient, topic)
